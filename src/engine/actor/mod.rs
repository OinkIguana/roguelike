...
pub trait Actor: ActorClone {
    ...
    /// Determines whether this `Actor` is able to enter a given `TileType`
    ...
    ///
    /// This method can be ignored if the `Actor` will not need to know its own location
...
/// `ActorClone` provides a utility for `Actor`s into `Box`es
pub trait ActorClone {
    /// Clones an `Actor` into a `Box`
